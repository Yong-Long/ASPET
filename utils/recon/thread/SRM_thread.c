...
#define PI 3.1415926
#define Tr 100.0
#define NUM_THREADS 6 // Number of threads to create
...
{
    return y < py_edge_up - pitch_half && y > py_edge_low &&
           z < pz_edge_up - pitch_half && z > pz_edge_low;
}
...
typedef struct ThreadData
{
    ...
    TRACING_CRY  **Y_alpha;
    TRACING_CRY  **Z_alpha;
    TRACING_CRY  **YZ_alpha_1;
    TRACING_CRY  **YZ_alpha_2;

    int   *tCOIN;
    float *pCOIN;
    float *pSparse;
    TABLE_ZIP     *pTable;
    INCREMENT_ZIP *pIncre;
    float *TOP_cone_resp;
    int   *TOP_cone_resp_tof;
} ThreadData;
...
{   // void *arg
    ...
    unsigned char tag_layers = 0;

    float lgridy = vy / ngrid;
    float lgridz = vz / ngrid;
    float lgridx = vx / ngrid;
    ...
    if (Y_alpha == NULL || Z_alpha == NULL)
    {
        fprintf(stderr, "Memory allocation failed for Y_alpha or Z_alpha\n");
    }
    if (Y_alpha[0] == NULL || Y_alpha[1] == NULL || Z_alpha[0] == NULL || Z_alpha[1] == NULL)
    {
        fprintf(stderr, "Memory allocation failed for Y_alpha[0], Y_alpha[1], Z_alpha[0], or Z_alpha[1]\n");
    }
    ...
    for (int ikernel = 0; ikernel < nkernel*nkernel; ikernel++)
    {
        ...
        // [Time] Start calculate time using of one kernel
        float T[2];
        T[0] = s_time();
    ...
    free(Y_alpha); free(Z_alpha);
    free(YZ_alpha_1); free(YZ_alpha_2);
    free(P1); free(P2);
    free(pTable); free(pIncre);
    free(tSparse); free(pSparse);
    free(tCOIN); free(pCOIN);
    free(TOP_cone_resp_tof); free(TOP_cone_resp);
    // pthread_exit(NULL);
}
...
int main (int argc, char *argv[])
{
    ...
    // - Allocate an array of ThreadData structures -
    // ThreadData *thData = (ThreadData *)malloc(NUM_THREADS * sizeof(ThreadData));
    
    // for (int i = 0; i < NUM_THREADS; i++)
    // {
    //     pthread_create(&threads[i], NULL, &threadFunction, NULL); //(void *)&thrData
    //     // pthread_join(threads[i], NULL);
    //     printf("[Info] Create threads (%d) success!\n", i);
    // }
    // for (int i = 0; i < NUM_THREADS; i++)
    // {
    //     pthread_join(threads[i], NULL);
